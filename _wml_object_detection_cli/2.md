...
>ダッシュボードから「リソースの表示」→「ストレージ」をクリックし、表示されたオブジェクトストレージをクリック後、Credentialsのステップに進んでください。
...
価格プランを選択し、**作成を** をクリックします。
![](assets/ic_storage_create.png)


## Credentials
オブジェクトストレージインスタンスを作成したら、IBM Cloudの外部からデータにアクセスできるようにする必要があります。このためには自分のリソースに対して認証情報（credentials）を作成する必要があります。
